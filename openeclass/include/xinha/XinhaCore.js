...
Xinha.version = {
  Release: 'Trunk',
  Head: '$HeadURL: http://svn.xinha.python-hosting.com/tags/0.92beta/XinhaCore.js $'.replace(
    /^[^:]*: (.*) \$$/,
    '$1'
  ),
  Date: '$LastChangedDate: 2007-02-22 02:11:56 +0100 (Do, 22 Feb 2007) $'.replace(
    /^[^:]*: ([0-9-]*) ([0-9:]*) ([+0-9]*) \((.*)\) \$/,
    '$4 $2 $3'
  ),
  ...
  if (Xinha.checkSupportedBrowser()) {
    ...
    if (this.config.showLoading) {
      ...
      loading_message.style.top =
        Xinha.findPosY(textarea) +
        parseInt(this._initial_ta_size.h, 10) / 2 +
        'px';
      // main static message
      ...
      loading_sub.appendChild(
        document.createTextNode(Xinha._lc('Constructing main object'))
      );
...
Xinha.Config = function () {
  ...
  this.btnList = {
    bold: [
      'Bold',
      Xinha._lc(
        { key: 'button_bold', string: ['ed_buttons_main.gif', 3, 2] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    italic: [
      'Italic',
      Xinha._lc(
        { key: 'button_italic', string: ['ed_buttons_main.gif', 2, 2] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    underline: [
      'Underline',
      Xinha._lc(
        { key: 'button_underline', string: ['ed_buttons_main.gif', 2, 0] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    strikethrough: [
      'Strikethrough',
      Xinha._lc(
        { key: 'button_strikethrough', string: ['ed_buttons_main.gif', 3, 0] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    subscript: [
      'Subscript',
      Xinha._lc(
        { key: 'button_subscript', string: ['ed_buttons_main.gif', 3, 1] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    superscript: [
      'Superscript',
      Xinha._lc(
        { key: 'button_superscript', string: ['ed_buttons_main.gif', 2, 1] },
        'Xinha'
      ),
      false,
      function (e) {
    ...
    cut: [
      'Cut selection',
      ['ed_buttons_main.gif', 5, 0],
      false,
      cut_copy_paste,
    ],
    copy: [
      'Copy selection',
      ['ed_buttons_main.gif', 4, 0],
      false,
      cut_copy_paste,
    ],
    paste: [
      'Paste from clipboard',
      ['ed_buttons_main.gif', 4, 1],
      false,
      cut_copy_paste,
    ],
    selectall: [
...
Xinha.prototype.generate = function () {
  ...
  if (typeof Dialog == 'undefined') {
    Xinha._loadback(
      _editor_url + 'modules/Dialogs/dialog.js',
      this.generate,
      this
    );
    return false;
  ...
  if (typeof Xinha.Dialog == 'undefined') {
    Xinha._loadback(
      _editor_url + 'modules/Dialogs/inline-dialog.js',
      this.generate,
      this
    );
    return false;
      ...
      switch (toolbar[i][j]) {
        ...
        case 'insertimage':
          if (
            ...
            return false;
          } else if (typeof InsertImage != 'undefined')
            editor.registerPlugin('InsertImage');
          break;
        case 'createlink':
          if (
            typeof CreateLink == 'undefined' &&
            typeof Xinha.prototype._createLink == 'undefined' &&
            typeof Linker == 'undefined'
          ) {
            Xinha.loadPlugin(
              'CreateLink',
              function () {
            ...
            return false;
          } else if (typeof CreateLink != 'undefined')
            editor.registerPlugin('CreateLink');
          break;
        case 'inserttable':
          if (
            typeof InsertTable == 'undefined' &&
            typeof Xinha.prototype._insertTable == 'undefined'
          ) {
            Xinha.loadPlugin(
              'InsertTable',
              function () {
            ...
            return false;
          } else if (typeof InsertTable != 'undefined')
        ...
        case 'forecolor':
          if (typeof ColorPicker == 'undefined') {
            ...
            return false;
          } else if (typeof ColorPicker != 'undefined')
  ...
  if (
    Xinha.is_gecko &&
    (editor.config.mozParaHandler == 'best' ||
      editor.config.mozParaHandler == 'dirty')
  ) {
    switch (this.config.mozParaHandler) {
      case 'dirty':
        var ParaHandlerPlugin =
          _editor_url + 'modules/Gecko/paraHandlerDirty.js';
        break;
      default:
        var ParaHandlerPlugin =
...
Xinha.prototype.initSize = function () {
  ...
  switch (this.config.height) {
    ...
    default:
      ...
      height = /[^0-9]/.test(this.config.height)
        ? this.config.height
...
Xinha.prototype.sizeEditor = function (
  ...
  if (!this._framework.tp_row.childNodes.length) {
    ...
    if (!Xinha.hasParentNode(this._framework.tp_row)) {
      this._framework.tbody.insertBefore(
        this._framework.tp_row,
        this._framework.ler_row
      );
    }
  }
  ...
  if (!this._framework.bp_row.childNodes.length) {
    ...
    if (!Xinha.hasParentNode(this._framework.bp_row)) {
      this._framework.tbody.insertBefore(
        this._framework.bp_row,
        this._framework.ler_row.nextSibling
      );
    }
  }
...
Xinha.loadPlugins = function (plugins, callbackIfNotReady) {
  ...
  while (nuPlugins.length) {
    ...
    if (typeof Xinha._pluginLoadStatus[p] == 'undefined') {
      ...
      Xinha.loadPlugin(p, function (plugin) {
        // @todo : try to avoid the use of eval()
...
Xinha.prototype._wordClean = function () {
  ...
  function checkEmpty(el) {
    ...
    if (
      /^(span|b|strong|i|em|font|div|p)$/i.test(el.tagName) &&
      !el.firstChild
    ) {
      Xinha.removeFromParent(el);
      ++stats.empty_tags;
...
Xinha.prototype.updateToolbar = function (noStatus) {
      ...
      case 'formatblock':
        ...
        if (deepestAncestor) {
          for (var x = 0; x < blocks.length; x++) {
            if (
              blocks[x].toLowerCase() == deepestAncestor.tagName.toLowerCase()
            ) {
  ...
  if (0 && Xinha.is_gecko) {
    ...
    if (
      ...
      s.anchorOffset == s.anchorNode.length &&
      !(
...
Xinha.prototype.fixRelativeLinks = function (html) {
  ...
  if (src) {
    ...
    for (var i = 0; i < src.length; ++i) {
      ...
      if (url_m) {
        ...
        html = html.replace(
          new RegExp(url[2].replace(Xinha.RE_Specials, '\\$1')),
          absPath
        );
      }
    }
  ...
  if (
    ...
    if (
      ...
      baseRe = new RegExp(
        '((href|src|background)=")(' +
          this.config.baseHref.replace(Xinha.RE_Specials, '\\$1') +
          ')',
        'g'
      );
    } else {
...
Xinha.checkSupportedBrowser = function () {
  if (Xinha.is_gecko) {
    if (navigator.productSub < 20021201) {
      alert(
        'You need at least Mozilla-1.3 Alpha.\nSorry, your Gecko is not supported.'
      );
      return false;
    }
...
Xinha.stripCoreCSS = function (html) {
  return html.replace(
    /<style[^>]+title="Xinha Internal CSS"(.|\n)*?<\/style>/i,
    ''
  );
};
...
Xinha.prototype.registerPlugins = function (plugin_names) {
  if (plugin_names) {
    for (var i = 0; i < plugin_names.length; i++) {
      this.setLoadingMessage('Register plugin $plugin', 'Xinha', {
...
Xinha.getElementTopLeft = function (element) {
  ...
  while (element) {
    ...
    if (
      ...
      element.offsetParent.tagName.toLowerCase() != 'body'
    ) {
...
Xinha.prototype.removeLoadingMessage = function () {
  if (
    !this.config.showLoading ||
    !document.getElementById('loading_' + this._textArea.name)
  ) {
    return;
  }
