...
switch ($c)	// get the case for each different listing
{
	case '':
		$view = 1;
		break; // normal listing
	case 'inactive':
		$view = 1;
		break; // normal listing. Display the inactive accounts
	case 'searchlist':
		$view = 2;
...
if (!empty($ord)) {
	switch ($ord) {
		case "s":
			$order = "statut,prenom,nom";
			break;
		case "n":
			$order = "nom,prenom,statut";
			break;
		case "p":
			$order = "prenom,nom,statut";
			break;
		case "u":
			$order = "username,statut,prenom";
			break;
		default:
			$order = "statut,prenom,nom";
			break;
	...
	if ($countUser > 0) {
		...
		for ($j = 0; $j < mysql_num_rows($sql); $j++) {
			while ($logs = mysql_fetch_array($sql, MYSQL_ASSOC)) {
				...
				switch ($logs['statut']) {
					case 1:
						$tool_content .= "<img src='../../template/classic/img/teacher.gif' title='$langTeacher'></img>";
						break;
					case 5:
						$tool_content .= "<img src='../../template/classic/img/student.gif' title='$langStudent'></img>";
						break;
					case 10:
						$tool_content .= "<img src='../../template/classic/img/guest.gif' title='$langVisitor'></img>";
						break;
					default:
						$tool_content .= "$langOther ($logs[6])";
						break;
				}
