...
if ($is_adminOfCourse) {       // teacher only
	...
	$safe_fileName = "20060301121510sdjklhsd.jpg"
        ...
        if (is_uploaded_file(@$userFile)) {
                ...
                if ($diskUsed + @$_FILES['userFile']['size'] > $diskQuotaDocument) {
                        ...
                        /*** Unzipping stage ***/
                        elseif (
                                isset($uncompress) and $uncompress == 1
	...
	RENAME
        ...
        if (isset($comment)) {
                ...
                $dialogBox .= "	<form method=\"post\" action=\"$_SERVER[SCRIPT_NAME]?edit_metadata\">
			...
			<option";
                if ($oldCategory == "0") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"0\">$langCategoryOther<br>";
                $dialogBox .= "	<option";
                if ($oldCategory == "1") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"1\">$langCategoryExcercise<br>
		<option";
                if ($oldCategory == "1") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"2\">$langCategoryLecture<br>
		<option";
                if ($oldCategory == "2") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"3\">$langCategoryEssay<br>
		<option";
                if ($oldCategory == "3") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"4\">$langCategoryDescription<br>
		<option";
                if ($oldCategory == "4") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"5\">$langCategoryExample<br>
		<option";
                if ($oldCategory == "5") $dialogBox .= " selected=\"selected\"";
                $dialogBox .= " value=\"6\">$langCategoryTheory<br>
		</select></td></tr>";
			...
			<td><input name='file_copyrighted' type='radio' value='0' ";
                if ($oldCopyrighted == "0" || empty($oldCopyrighted)) $dialogBox .= " checked='checked' ";
                $dialogBox .= " /> $langCopyrightedUnknown <input name='file_copyrighted' type='radio' value='2' ";
                if ($oldCopyrighted == "2") $dialogBox .= " checked='checked' ";
                $dialogBox .= " /> $langCopyrightedFree <input name='file_copyrighted' type='radio' value='1' ";

                if ($oldCopyrighted == "1") $dialogBox .= " checked='checked' ";
                $dialogBox .= "/> $langCopyrightedNotFree
    		</td></tr>
...
if ($is_adminOfCourse) {
	...
	Create new folder
	--------------------------------------*/
        $tool_content .= "\n      <li><a href='$_SERVER[SCRIPT_NAME]?createDir=" . $cmdCurDirPath . "'>$langCreateDir</a></li>";
...
if (mysql_num_rows($sql) == 0) {
        ...
        function headlink($label, $this_sort)
        {
                ...
                return '<a href=\'' . $_SERVER['SCRIPT_NAME'] . '?openDir=' . $path .
        ...
        {
                $parentlink = $_SERVER['SCRIPT_NAME'] . '?openDir=' . $cmdParentDir;
        ...
        foreach (array(true, false) as $is_dir) {
                foreach ($fileinfo as $entry) {
                        ...
                        if ($is_dir) {
                                ...
                                $file_url = "$_SERVER[SCRIPT_NAME]?openDir=$cmdDirName";
                                $link_extra = '';
