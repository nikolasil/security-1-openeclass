...
';
		...
		if (mysql_num_rows($result) > 1) {
			if (isset($_GET["sens"]) && $_GET["sens"] == "d") {
				$tool_content .=  "\n<li><a href='$_SERVER[SCRIPT_NAME]?sens=' >$langOldToNew</a></li>";
				$sens = " DESC ";
			} else {
				$tool_content .=  "\n<li><a href='$_SERVER[SCRIPT_NAME]?sens=d' >$langOldToNew</a></li>";
			}
		}
		$tool_content .= "\n    </ul>\n  </div>\n";
	}
	...
	if (isset($_GET['addEvent']) or isset($_GET['edit'])) {
		...
		$navigation[] = array("url" => $_SERVER['SCRIPT_NAME'], "name" => $langAgenda);
		$tool_content .= "
<form method='post' action='$_SERVER[SCRIPT_NAME]' onsubmit='return checkrequired(this, \"titre\");'>
...
if (mysql_num_rows($result) > 0) {
	...
	while ($myrow = mysql_fetch_array($result)) {
		...
		<a href='$_SERVER[SCRIPT_NAME]?id=" . $myrow['id'] . "&amp;edit=true'>
            	<img src='../../template/classic/img/edit.gif' border='0' title='" . $langModify . "'></a>&nbsp;
        	<a href='$_SERVER[SCRIPT_NAME]?id=" . $myrow[0] . "&amp;delete=yes' onClick='return confirmation('" . addslashes($myrow["titre"]) . "');'>
            	<img src='../../template/classic/img/delete.gif' border='0' title='" . $langDelete . "'></a>&nbsp;";
			if ($myrow["visibility"] == 'v') {
				$tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?id=" . $myrow[0] . "&amp;mkInvisibl=true'>
        	    	<img src='../../template/classic/img/visible.gif' border='0' title='" . $langVisible . "'></a>";
			} else {
				$tool_content .= "<a href='$_SERVER[SCRIPT_NAME]?id=" . $myrow[0] . "&amp;mkVisibl=true'>
