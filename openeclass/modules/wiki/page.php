...
if ($is_allowedToEdit || $is_allowedToCreate) {
    $valid_actions = array(
        "edit", "preview", "save", "show", "recent", "diff", "all", "history"
    );
} else {
    $valid_actions = array(
            ...
            if (isset($content)) {
                ...
                if ($wikiPage->pageExists($title)) {
                    ...
                    if ($content == $wikiPage->getContent()) {
                        ...
                        if (
                            isset($_SESSION['wikiLastVersion'])
...
switch ($action) {
    case "edit": {
            ...
            $navigation[] = array(
                'url' => 'page.php?action=show&amp;wikiId='
                    . $wikiId . '&amp;title=' . $title, 'name' => $dispTitle
            );
...
switch ($action) {
    ...
    case "diff": {
            ...
            $oldTime = claro_format_locale_date(
                $dateTimeFormatLong,
                strtotime($oldTime)
            );
            ...
            $newTime = claro_format_locale_date(
                $dateTimeFormatLong,
                strtotime($newTime)
            );
    ...
    case "edit": {
            if (!$wiki->pageExists($title) && !$is_allowedToCreate) {
                ...
                $tool_content .= claro_disp_wiki_editor(
                    $wikiId,
                    $title,
                    $versionId,
                    $content,
    ...
    case "show": {
            if ($wikiPage->hasError()) {
                ...
                if ($versionId != 0) {
                    ...
                    $mtime = claro_format_locale_date(
                        $dateTimeFormatLong,
    ...
    default: {
            trigger_error(
                "Invalid action supplied to " . $_SERVER['SCRIPT_NAME'],
                E_USER_ERROR
            );
        }
}
