...
elseif (isset($_REQUEST['delete_one']) && $is_adminOfCourse) {
	...
	while ($myDir = mysql_fetch_array($sqlDir)) {
		rename(
			"../../courses/$currentCourse/group/$myDir[secretDirectory]",
...
if ($is_adminOfCourse) {
	...
	// $group_creation_csrf_token = get_sessions_csrf_token('csrf_token_group_creation');
	// $group_edit_csrf_token = get_sessions_csrf_token('csrf_token_group_edit');
