...
if($is_adminOfCourse) {
	...
	while ($row = mysql_fetch_array($result)) {
		if(isset($fromExercise) || !is_object(@$objExercise) || !$objExercise->isInList($row['id'])) {
			...
			if(!isset($fromExercise)) {
				...
				$tool_content .= "<a href=\"".$_SERVER['SCRIPT_NAME']."?recup=".$row['id'].
			...
			if(!isset($fromExercise)) {
				$tool_content .= "<td><div align='center'>
					<a href=\"".$_SERVER['SCRIPT_NAME']."?exerciseId=".$exerciseId."&delete=".$row['id']."\"". 
	...
	if ($numpages > 0) {
		...
		if ($page > 0) {
			$prevpage = $page-1;
			if (isset($fromExercise)) {
				$tool_content .= "<small>&lt;&lt; <a href=\"".$_SERVER['SCRIPT_NAME'].
				"?exerciseId=".$exerciseId.
				"&fromExercise=".$fromExercise.
				"&page=".$prevpage."\">".$langPreviousPage."</a></small>";
			} else {
				$tool_content .= "<small>&lt;&lt; 
				<a href='$_SERVER[SCRIPT_NAME]?page=$prevpage'>$langPreviousPage</a></small>";
			}
		}
		if ($page < $numpages) {
			$nextpage = $page+1;
			if (isset($fromExercise)) {
				...
				$tool_content .= "<small>
				<a href='$_SERVER[SCRIPT_NAME]?page=$nextpage'>$langNextPage</a> &gt;&gt;
				</small>";
			}
		}
	}	 
