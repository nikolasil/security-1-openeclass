...
function show_lp($title, $comments, $resource_id, $lp_id)
{
        ...
        $title = htmlspecialchars($title);
        $r = db_query(
                "SELECT * FROM lp_learnPath WHERE learnPath_id = $lp_id",
                $currentCourseID
        );
...
function show_work($title, $comments, $resource_id, $work_id, $visibility)
{
        ...
        $title = htmlspecialchars($title);
        $r = db_query(
                "SELECT * FROM assignments WHERE id = $work_id",
                $currentCourseID
        );
...
function show_exercise($title, $comments, $resource_id, $exercise_id, $visibility)
{
        ...
        $title = htmlspecialchars($title);
        $r = db_query(
                "SELECT * FROM exercices WHERE id = $exercise_id",
                $currentCourseID
        );
        if (mysql_num_rows($r) == 0) { // check if it was deleted
                if (!$is_adminOfCourse) {
...
function show_wiki($title, $comments, $resource_id, $wiki_id, $visibility)
{
        ...
        $title = htmlspecialchars($title);
        $r = db_query(
                "SELECT * FROM wiki_properties WHERE id = $wiki_id",
                $currentCourseID
        );
        if (mysql_num_rows($r) == 0) { // check if it was deleted
                if (!$is_adminOfCourse) {
