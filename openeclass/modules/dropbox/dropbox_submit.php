...
if (isset($_POST["dropbox_unid"])) {
	...
	$dropbox_unid = htmlspecialchars($_GET["dropbox_unid"]);
} else {
...
if (isset($_POST["submitWork"])) {
	...
	if (!$error) {
		...
		if (!$error) {
			...
			$dropbox_filename = bin2hex(openssl_random_pseudo_bytes(32)) . $dropbox_filename;
			if (!$error) {
...
if (isset($_GET['mailingIndex']))  // examine or send
{
	...
	$i = htmlspecialchars($_GET['mailingIndex']);
	...
	if (!preg_match($dropbox_cnf["mailingZipRegexp"], $mailing_title, $nameParts)) {
		...
		if ($zipContent) {
			foreach ($zipFile->listContent() as $thisContent) {
				...
				if (preg_match("~.(php.*|phtml)$~i", $thisFile)) {
					$error = TRUE;
					$errormsg .= $thisFile . ': ' . $dropbox_lang["mailingZipPhp"];
					break;
				} elseif (!$thisContent['folder']) {
					if ($ucaseFiles[$thisFileUcase]) {
						$error = TRUE;
						$errormsg .= $thisFile . ': ' . $dropbox_lang["mailingZipDups"];
						break;
					} else {
		...
		$error = TRUE;
		$errormsg .= $dropbox_lang["mailingWrongZipfile"];
	}
